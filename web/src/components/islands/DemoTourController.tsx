  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-2">
      <button
        ...
        disabled={running}
        className="group relative inline-flex items-center gap-3 px-6 py-3.5 rounded-2xl bg-gradient-to-r from-cyan-500 via-teal-400 to-emerald-400 hover:from-cyan-400 hover:to-emerald-300 text-slate-950 font-extrabold text-sm sm:text-base shadow-xl shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all transform hover:-translate-y-0.5 active:translate-y-0 cursor-pointer disabled:opacity-80"
      >
        <span className="text-lg group-hover:scale-125 transition-transform">🎬</span>
        <span>{running ? 'Running Guided Demo Tour...' : 'Run 60s Live Demo Tour'}</span>
      ...
      {stepText && (
        <div className="p-3 bg-slate-900/90 border border-cyan-500/40 rounded-xl text-cyan-300 font-mono text-xs animate-pulse max-w-md text-center shadow-lg">
          {stepText}
        </div>
      )}
    </div>
