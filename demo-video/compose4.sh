...
WEBM="$CLIPS/page@6b55868e332f93f57a719ed15c2c2818.webm"
...
# Split raw footage (99.5s total):
# 0-15s:    landing page
# 15-90s:   rehearsal autoplay — full flow with booking (75s)
# 90-99.5s: demo receipt page (10s)
echo "Splitting raw footage into segments..."
ffmpeg -y -i "$MP4" -ss 0  -t 15 -c copy "$CLIPS/seg-landing.mp4"    2>/dev/null
ffmpeg -y -i "$MP4" -ss 15 -t 75 -c copy "$CLIPS/seg-rehearsal.mp4"  2>/dev/null
ffmpeg -y -i "$MP4" -ss 90 -t 10 -c copy "$CLIPS/seg-receipt.mp4"    2>/dev/null
echo "  3 segments split"
