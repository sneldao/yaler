...
| 1:00 | Open `/missions/new`. Press **/** to jump to the job box, tap **Use a sample job**, submit. Watch DRAFT → rules → SOURCING → quotes land. | "That's a real job on the real API. Three independent supplier agents — each powered by Gemini — role-play different London businesses and generate quotes in their own voice." | If creation fails, open a seeded job instead: `/missions/seed-mission-offers-01` (quotes in) or `/missions/seed-mission-sourcing-01` (still asking). |
...
| 2:45 | Open `/ops` — the concierge desk. | "Verified engineers get real callouts; a human concierge works this queue — WhatsApp out, typed replies in. The AI supplier agents keep demos moving and every quote is labelled." | The desk shows its quiet empty state — that *is* the fallback, and it's honest. |
